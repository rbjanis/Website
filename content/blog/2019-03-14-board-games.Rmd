...
This is my first time participating in Tidy Tuesday AND my first blog post. My love for board games finally convinced me to actually use my blog. 
...
The `cowplot` package came to the rescue in getting all four plots into one figure. Most games require a minimum of two players, and the median play time is `r median(board_games$playing_time)` minutes. The plot only shows games <=10 hours, but there were `r sum(board_games$playing_time > 600)` board games with play time over 10 hours. These are likely a mix of games played over multiple sessions and erroneous data. 
...
Most rated and highest average rated game in each of the top 10 mechanics, to get a sense for what types of games they are. 
...
One important metric for frequent itemsets is **support**, which indicates how frequently the itemset appears in the dataset, or the proportion of games that contain the combination of mechanics. 

Below are the 10 mechanic combinations with the highest support. The most frequent combination of mechanics is Dice Rolling and Variable Player Powers, which occurred in about 5% of all games. This combination makes sense, because those two mechanics occurred with high frequency (they were the 1st and 4th highest, respectively). Interestingly, the combination of the two most frequent mechanics, Dice Rolling and Hand Management, only occurs 6th most frequently as a pair.
...
Like with frequent itemsets, we can look to support to indicate the frequency of the left hand side item set in the data. Association rule mining employs an additional metric, **confidence**, which indicates how often the rule is true, or the proportion of games that have the LHS that also contain the RHS. Crayon Rail System only appears in <1% of games, but 82% of the time a game is labled as Crayon Rail System, it is also labeled as Route/Network Buiding and Pick-up and Deliver, with this association rule ocurring in 14 games.  

```{r}
inspect(association_rules[1:11])
```

Below are the 10 association rules with the higest confidence. For the first 5, in 100% of the games that the combination of left hand side mechanics occur, the right hand side mechanic occurs as well. So 100% of the games labeled as Auction/Bidding, Stock Holding, and Tile Placement are also labeled as Route/Network Building, and this association occurs in 23 games. 

```{r}
inspect(sort(association_rules, by = "confidence")[1:10])
```
